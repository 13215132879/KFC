...
🤖 主要功能：
❶将其他格式的⟦服务器 订阅⟧解析成 𝐐𝐮𝐚𝐧𝐭𝐮𝐦𝐮𝐥𝐭𝐗 格式 
☑︎ 支持 2𝐫𝐚𝐲𝐍 / 𝗦𝗦 （𝗥 / 𝗗 ）/ 𝗛𝗧𝗧𝗣 （𝗦 ）/ 𝗧𝗿𝗼𝗷𝗮𝗻 / 𝗤𝘂𝗮𝗻𝘁𝘂𝗺𝘂𝗹𝘁 （𝗫 ）/ 𝗦𝘂𝗿𝗴𝗲 / 𝐂𝐥𝐚𝐬𝐡 /𝐒𝐡𝐚𝐝𝐨𝐰𝐫𝐨𝐜𝐤𝐞𝐭 /𝐋𝐨𝐨𝐧 / 𝐋𝐨𝐨𝐧 / 𝐋𝐨𝐨𝐧 格式
☑︎ 提供说明 1⃣️中 任选个性化参数（筛选、重命名 等）
❷ 𝗿𝗲𝗲𝗿𝗶𝘁𝗲 （采用） &  𝗳𝗶𝗿𝘁𝗲𝗿𝘁𝗲𝗿 （分流） 的 转换 和 筛选（分流） 
☑︎ 禁止/修改远程引用中某(几)项 𝗿𝗲𝘄𝗿𝗶𝘁𝗲 / 𝗵𝗼𝘀𝘁𝗻𝗮𝗺𝗲 / 𝗳𝗶𝗹𝘁𝗲𝗿
☑︎ 𝐒𝐮𝐫𝐠𝐞 / 𝐂𝐥𝐚𝐬𝐡 类型规则 𝗹𝗶𝘀𝘁 与 模块 𝐦𝐨𝐝𝐮𝐥𝐞 的解析使用
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
0️⃣在⟦ 订阅链接⟧后加“#”使用，不同参数用“ & ”连接     
⚠️☞“你的订阅连接# emoji = 1 & tfo = 1 & in =香港+台湾”
❖ 本地资源截图引用, 参数参数如#in = xxx & out = yyy ” 填入资源截图的第①行
❖ 🚦 支持中文, "操作"以下特殊字符时请先替换(URL-Encode)🚦
  ∎ " + "⇒" % 2 B ", 空格⇒" % 20 " ,  "@" ⇒ "%40" ,  "&" ⇒ "%26" ,  "." ⇒ “\。” ,  "," ⇒ "%2C"

1️⃣⟦𝐬𝐞𝐫𝐯𝐞𝐫节点⟧➠ _  _  参数说明：
⦿ji  = 1 (国行设备用2 ) , /删除节点名内表情/表情; 
⦿  udp = 1 / - 1 ,  tfo = 1 / - 1 , 分别强制开启(关闭) 𝐮𝐝𝐩 - 𝐫𝐞𝐥𝐚𝐲 / 𝐟𝐚𝐬𝐭 - 𝐨𝐩𝐞𝐧 ;
⦿ uot  = 1 ,开启 udp  - over tcp = true选项（接入SS ( R )）
⦿  cert = 1 / - 1 , 分别开启/关闭 𝐭𝐥𝐬 证书验证（默认）；
  ❖ csha / psha ,  tls - cert - sha256 以及 tls - pubkey - sha256 参数
  ❖  alpn , 指定over - tls参数类型的alpn
⦿  in ,  out ,  regex ,  regout 分别为 保留、删除、正则保留、正则删除 节点；
  ❖  in / out 仅对节点名匹配生效，多参数( "或" ) " + "，逻辑逻辑"与""。" 表示；
  ❖ 正则表达式/ regout 对节点的完整信息进行匹配（类型、端口、加密等）；
  ❖示范: "in=香港.0\.2倍率+台湾&out=BGP®ex=iplc"
⦿ 重 命名、 “旧名@新名” 、 “知名@” 、 “@后缀” 、“  + ”连接多个；
  ❖ 删除场地：“替换场地1。场地2☠️ ”，想删除“ 。”“时用”“ \。”
  ❖ 示范：" rename =香港@ 𝐇𝐊 + [ 𝐒𝐒 ] @ + @ [ 1 𝐗 ] +流量.0 \. 2 ☠️ "
  ❖ 表情符号先生效，如想换调顺序，请使用 rrname 参数
⦿ 正替换节点中替换，可用于重命名/更改加密方式等
  ❖ 替换=regex1@𝘀𝘁𝗿1+regex2@𝘀𝘁𝗿2
⦿ ptn/npt=1-8, 将节点名英文/数字替换成样式⇒ 🅰/🄰/𝐀/𝗮/𝔸/𝕒/ᵃ/ᴬ, ①\❶\⓵\𝟙\¹\₁\𝟏\𝟷
⦿ delreg，使用正则表达式来删除“节点名”中的字段（⚠️慎用）
⦿ aead=-1, 关闭 Vmess 的 AEAD 参数
⦿ host=xxx, 已有主机，如要增加主机，请用☠️结尾
⦿ checkurl=xxx , 指定 server_check_url 参数
⦿ sort=1/-1/x/参数规则，按节点名正/逆/随时/参数规则示例
  ❖ 参数规则是正则表达式或关键词，用" < "简单或" > "连接
  ❖ sort=🇭🇰>🇸🇬>🇯🇵>🇺🇸 , 靠前示例
  ❖ sort IEPL<IPLC<BGP , 靠后示例
⦿ info=1, 开启通知提示机场 ✈️ 流量信息（如有）；
⦿flow=2022-06:10:54, 00-订阅时间0: 总流量:00 订阅时间: 总流量: 已用
⦿ 占位符，可用于重命名/替换等操作
